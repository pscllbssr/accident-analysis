...
accidents_df[,c("AccidentUID", "AccidentWeekDay", "AccidentHour")] %>%
  ...
  summarise(Count = n()) %>% 
  ggplot(aes(x = AccidentWeekDay, y = AccidentHour, fill = Count)) +
    geom_raster() +
...
accidents_df[,c("AccidentLocation_CHLV95_E", "AccidentLocation_CHLV95_N")] %>%
  ggplot(aes(x = AccidentLocation_CHLV95_E, y = AccidentLocation_CHLV95_N)) +
